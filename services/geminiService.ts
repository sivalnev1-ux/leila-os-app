  ...
  async sendMessage(
    ...
    try {
      ...
      if (department === Department.FINANCE) {
        deptContext += `\nФУНКЦИЯ OCR (РАСПОЗНАВАНИЕ ТЕКСТА): Если пользователь прикрепляет изображение чека/квитанции, твоя задача — автоматически распознать текст и извлечь данные. ВАЖНО: НИКОГДА НЕ ОБЪЕДИНЯЙ ТОВАРЫ В ОБЩУЮ СУММУ ЧЕКА! Ты ОБЯЗАНА вытащить КАЖДУЮ ОТДЕЛЬНУЮ ПОЗИЦИЮ из чека (каждый уникальный товар, вкус, вид, количество, цену) и вызвать функцию \`insert_into_sheet\` для КАЖДОЙ ПОЗИЦИИ отдельно новой строкой! Если в накладной 10 разных кормов, ты должна вызвать функцию 10 раз, чтобы пользователь мог вести точную аналитику по конкретным товарам. КРИТИЧЕСКИ ВАЖНО: ЗАПРЕЩАЕТСЯ СПРАШИВАТЬ РАЗРЕШЕНИЯ У ПОЛЬЗОВАТЕЛЯ. НЕМЕДЛЕННО ВЫПОЛНЯЙ ФУНКЦИЮ \`insert_into_sheet\` ДЛЯ ВСЕХ ТОВАРОВ. КРОМЕ ТОГО, КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ ПЕЧАТАТЬ ИЗВЛЕЧЕННЫЕ ТОВАРЫ В ВИДЕ ТЕКСТА ИЛИ ТАБЛИЦЫ В ТВОЙ ОТВЕТ! Всю информацию передавай ТОЛЬКО внутрь аргументов функции insert_into_sheet. В текстовом ответе просто скажи 1 предложением: "Чек обработан, товары отправлены в базу."`;
