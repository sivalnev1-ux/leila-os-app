...
async function testGenerative() {
    ...
    try {
        // API v2 endpoint for generative changes according to user's screenshot
        ...
        if (!response.ok) {
            ...
            fs.writeFileSync(`test_generative.jpg`, Buffer.from(buffer));
            console.log(`Saved test_generative.jpg. File size:`, fs.statSync('test_generative.jpg').size);
        }
    } catch (e) {
        console.error(e);
    }
